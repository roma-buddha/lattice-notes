...
export type Provider =
  "openrouter" | "google" | "nvidia" | "custom" | "lotus" | "local";
...
export const ai = {
  ...
  computerSpecs: (refresh = false) =>
    invoke<ComputerSpecs>("ai_computer_specs", { refresh }),
  library: () => invoke<ModelLibrary>("model_library"),
  chooseModelRoot: () =>
    invoke<ModelLibrary | null>("model_library_choose_root"),
  ...
  renameModel: (path: string, name: string) =>
    invoke<LibraryFile>("model_library_rename", { path, name }),
  moveModel: (path: string, targetRoot: string) =>
    invoke<LibraryFile>("model_library_move", { path, targetRoot }),
  deleteModel: (path: string) => invoke<void>("model_library_delete", { path }),
  hfSearch: (query: string) =>
    invoke<{ id: string; downloads: number; likes: number }[]>("hf_search", {
      query,
    }),
  hfFiles: (repository: string) =>
    invoke<{ path: string; size: number }[]>("hf_files", { repository }),
  hfDownload: (repository: string, file: string, destinationName: string) =>
    invoke<void>("hf_download", { repository, file, destinationName }),
